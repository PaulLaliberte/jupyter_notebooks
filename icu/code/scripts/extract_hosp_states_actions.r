...
for (index in 1:21) {
    ...
    states = cbind(rowSums(data_ED[,2:9]), rowSums(data_ED[,10:12]), rowSums(data_ICU[,2:5]))
    ...
    write.table(states, paste("/home/pl2669/Desktop/ccp/extracted_data/states_actions_ICU_include_all/no_dropping_zeros/all/HOSP_",hosp,"_observed_states.txt", sep=""), row.names=F, col.names=F)

    write.table(actions, paste("/home/pl2669/Desktop/ccp/extracted_data/states_actions_ICU_include_all/no_dropping_zeros/all/HOSP_",hosp,"_observed_actions.txt", sep=""), row.names=F, col.names=F)
}
