...
for (index in 1:21) {
    ...
    #(low_ED, ICU_all, externals_{high, other_externals})
    states = cbind(rowSums(data_ED[,2:9]), rowSums(data_ICU[, c("ICU_ED_Medical", "ICU_ED_Surgical", "ICU_non_ED_Medical", "ICU_non_ED_Surgical")]), rowSums(data_ICU[, c("ICU_ED_Surgical", "ICU_non_ED_Medical", "ICU_non_ED_Surgical")]))
    #actions = cbind(rowSums(data_ED[,seq(from=24,to=38,by=2)]), rowSums(data_ED[,seq(from=25,to=39,by=2)]), rowSums(data_ED[,seq(from=40,to=44,by=2)]), rowSums(data_ED[,seq(from=41,to=45,by=2)]))
    actions = cbind(rowSums(data_ED[,seq(from=24,to=38,by=2)]), rowSums(data_ED[,seq(from=25,to=39,by=2)]))

    write.table(states, paste("/home/pl2669/Desktop/ccp/extracted_data/states_actions_ICU_include_all/no_dropping_zeros/low/HOSP_",hosp,"_observed_states.txt", sep=""), row.names=F, col.names=F)

    write.table(actions, paste("/home/pl2669/Desktop/ccp/extracted_data/states_actions_ICU_include_all/no_dropping_zeros/low/HOSP_",hosp,"_observed_actions.txt", sep=""), row.names=F, col.names=F)
}
